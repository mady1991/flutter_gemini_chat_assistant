...
// Method to check local database first
  Future<String> hybridResponse(String userMessage) async {
    // This would integrate with your local JSON search
    // For now, returns empty string if not found locally
    return ''; // Return local solution if found, empty string if not
  }

// Method to handle hybrid chat
  Future<void> sendHybridMessage({
    required String message,
    required bool isTextOnly,
  }) async {
    // First check local database
    final localSolution = await hybridResponse(message);

    if (localSolution.isNotEmpty) {
      // Add local solution to chat
      _inChatMessages.add(Message(
        messageId: DateTime.now().millisecondsSinceEpoch.toString(),
        chatId: _currentChatId,
        role: Role.assistant,
        message: StringBuffer("📱 **Local Solution Found:**\n\n$localSolution"),
        imagesUrls: [],
        timeSent: DateTime.now(),
      ));
    } else {
      // Fall back to Gemini AI
      await sentMessage(message: message, isTextOnly: isTextOnly);
    }
    notifyListeners();
  }
}
